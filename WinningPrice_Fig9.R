...
#   browser()
  ...
  { # lm on win
    ...
    progressive.cv <- list()
    for(lambda2 in c(1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000)) {
      ...
      progressive.cv[[paste(lambda2)]] <- mseloss(y.next[["win"]], l.win_lm$predict(m1.next[["win"]]))
    }
    lambda2 <- unlist(progressive.cv) %>% which.min %>% names %>% as.numeric
    loginfo("Selected lambda2: %d", lambda2)
    l.win_lm <- linear_regression(m1$win, y$win, lambda2, start = .start)
  }
  { # lm on loose
...
for(wp.ratio.i in seq_along(wp.ratio.all)) {
  ...
  for(season in c("training3rd")) {
    for(i in head(seq_along(day.list[[season]]), -1)) {
      ...
      bidimpclk.next <- readRDS(sprintf("cache/bidimpclk.%s.%d.sim2.Rds", format(day + 1, "%Y%m%d"), wp.ratio.i))
      ...
      saveRDS(r1, sprintf("cache/exp.%s.%d.model1.Rds", day, wp.ratio.i))
    }
  }
}
